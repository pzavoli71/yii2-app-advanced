...
<div class="quiz-index">
    ...
    <?php
		...
		<tr >
			...
			<th data-nomecol='nomegruppo'>nomegruppo</th>
		...
		foreach ($models as $riga) {?>
			<tr id='Rigazgruppo_<?=$pos?>' chiave="<?=$riga->idgruppo?>" class="<?=fmod($pos,2) == 1?'rigaDispari':'rigaPari'; ?>">
				<td class="tdbottonifrontali"><?= showToggleInrelations($riga,$pos,true) ?>
					<?php echo frontend\controllers\BaseController::linkwin('Edit|fa-edit', 'abilitazione/zgruppo/view', ['idgruppo'=>$riga->idgruppo], 'Apri per modifica','document.location.reload(false)',['windowtitle'=>'Inserisci i parametri','windowwidth'=>'700']); ?>
				</td>   
				<td><span class="headcol">idgruppo:</span><?= $riga->idgruppo ?></td>
				<td><span class="headcol">nomegruppo:</span><?= $riga->nomegruppo ?></td>
...
//    Righe Relazione                             
// ============================================ 
function Relazionizgruppo($riga, $rigapos) { ?>
...
function Relazionezgruppo_zutgr($riga, $rigapos, $loadable = false) { ?>
	...
	<table border="0" cellpadding="2" cellspacing="0" class="tabLista" id="tabListazgruppo_zutgr_<?=$rigapos?>" nomepdc="zgruppo">
		<?= IntestaTabellazutgr()?>
		<?php $p=1; if ( $loadable)
			foreach ($riga->zutgr as $value) {
				Recordzutgr($value,$p);
				$p++;
			}?>		
	</table>
